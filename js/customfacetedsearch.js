    ...
    function showresults(obj,val)
    {
        ...
        queries = {'c':"select ?paper  ?key ?value ?label where {?paper <http://data.semanticweb.org/ns/swc/ontology#isPartOf> ?book. ?book <http://swrc.ontoware.org/ontology#booktitle> \""+searchdata+"\" . ?paper <http://purl.org/dc/elements/1.1/title> ?title . ?paper ?key ?value. ?key rdfs:label ?label . values (?label ?pred) {  (\"Month\" 4)  (\"Citation\" 7) (\"Abstract\" 2) (\"Paper Title\" 1) (\"Article Body\" 6) (\"Year\" 5) (\"Maker\" 3) } . } order by ?paper ?pred",
                   'a':"select ?paper  ?key ?value ?label where {?o <http://xmlns.com/foaf/0.1/name> \""+searchdata+"\" . ?paper <http://xmlns.com/foaf/0.1/maker>  ?o .   ?paper <http://purl.org/dc/elements/1.1/title> ?title .   ?paper ?key ?value. ?key rdfs:label ?label . values (?label ?pred) {  (\"Month\" 4)  (\"Citation\" 7) (\"Abstract\" 2) (\"Paper Title\" 1) (\"Article Body\" 6) (\"Year\" 5) (\"Maker\" 3) } . } order by ?paper ?pred" ,
                   'o':"select ?paper  ?key ?value ?label where { ?person <http://swrc.ontoware.org/ontology#affiliation> ?univuri . ?univuri rdfs:label \""+searchdata+"\" . ?person  <http://xmlns.com/foaf/0.1/made> ?paper . ?paper <http://purl.org/dc/elements/1.1/title> ?title .  ?paper ?key ?value. ?key rdfs:label ?label . values (?label ?pred) {  (\"Month\" 4)  (\"Citation\" 7) (\"Abstract\" 2) (\"Paper Title\" 1) (\"Article Body\" 6) (\"Year\" 5) (\"Maker\" 3) } . } order by ?paper ?pred"
                  }
        data = {'query':queries[val],'bindingset':'paper,label,value,key','repository':'ou'}
        console.log(queries[val])
        url = "http://"+window.location.host+"/CL/First.jsp"
        xhr = $.ajax({
                url:url,
                type:'POST',
                data:data,
        ...
        $('#d').html('<img src="load.gif" height="50px" width="50px"/><br /><br />');
    }
            ...
            for(var j = 1;j<trs.length;j++)
            {
                ...
                for(var k = 1;k<3;k++ )     
                {
                    ...
                    {    
                        ...
                        txt += '<img class="rowload" title = "Looking up database" src="load.gif"/>'                        
                    }
                    else if(!checked)
                    {
